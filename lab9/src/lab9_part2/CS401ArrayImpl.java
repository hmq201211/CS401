...
public class CS401ArrayImpl<E> implements CS401CollectionInterface<E> {
	...
	public E remove(int i) {
		/*
		 * Add code here. Remember to compact the array so there are no spaces in
		 * between if an element from the middle is removed or an element at the
		...
		if (i < 0 && i > num_elements)
			...
			toReturn = elements[i];
			for (int k = i + 1, j = i; k < num_elements; k++, j++) {
				E temp = elements[j];
	...
	public boolean add(Where where, int index, E e) {
		 ...
		 */
		boolean flag = false;
		if (where == Where.MIDDLE && index > 0 && index < num_elements - 1) {
			for (int i = num_elements - 1, j = i + 1; i >= index; i--, j--) {
				E temp = elements[i];
	...
	public boolean add(Where where, E e) {
		...
		if (where == Where.BACK) {
			...
			/*
			 * Add code here. You will add the new element at index 0, and shift all the
			 * elements down by one.
			 */
			for (int i = num_elements - 1, j = i + 1; i >= 0; i--, j--) {
				E temp = elements[i];
				elements[i] = elements[j];
				elements[j] = temp;
			}
	...
	private boolean grow() {
		...
		/*
		 * Add code here Expand capacity (double it) and copy old array contents to the
		 * new one.
		 */
