...
public class CS401BinaryTree<E> {
	...
	private int index; /*
						 * This is the index of the next node to insert in the tree.
						 */
	...
	private int size_p(TreeNode<E> t) {
		 ...
		 */
		if (t != null)
			num_nodes = 1 + size_p(t.left) + size_p(t.right);
		return num_nodes;
	}
	...
	/*
	 * ------------------------------------------------------------------- Adds a
	 * node to the binary tree. Nodes are added from the left. Thus, each level in
	 * the tree is fleshed out before a new level is started.
	 */
	public void add(E info) {
		/*
		 * Additions are done from the leftmost. Thus, the first node (index 0) is the
		 * root; the second node (index 1) is the left child of the root, the third node
		 * (index 2) is the right child of the root. The fourth node (index 3) will be
		 * the left child of the node at index 1. And so on.
		 *
		 * To give you some context, the node having index 0 occurs at level 0, the
		 * nodes having indexes 1 and 2 occur at level 1, the nodes having index 3,4,5
		 * occur at level 2.
		 */
	...
	/*
	 * ------------------------------------------------------------------- Each node
	 * in the tree is an object of this type.
	 */
